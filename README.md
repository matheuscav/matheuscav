<h1 align="center">🗡️ Matheus — A Jornada do Analista 🗡️</h1>

<p align="center">
  <img src="https://readme-typing-svg.demolab.com/?font=Press+Start+2P&size=16&duration=3000&pause=1000&color=8A2BE2&center=true&vCenter=true&width=600&lines=Carregando+aventura...;Classe%3A+Analista+de+Sistemas+em+forma%C3%A7%C3%A3o;Miss%C3%A3o+atual%3A+deploy+do+bot+financeiro" alt="Typing SVG" />
</p>

---

## 🧙 Ficha de Personagem

| Atributo | Valor |
|---|---|
| 🎮 Classe | Analista de Sistemas / Dados (em transição) |
| ⭐ Nível | Iniciante → Pleno |
| 🎯 Missão atual | Deploy do `whatsapp-financeiro` na Oracle Cloud |
| 🛡️ Guilda | Autodidatas & Fundação Bradesco (Escola Virtual) |
| 📍 Base | Brasília, BR |

---

## ⚡ Barra de XP

![XP](https://progress-bar.dev/65/?title=Progresso%20da%20transi%C3%A7%C3%A3o%20de%20carreira&width=400&color=8A2BE2)

---

## 🗺️ Mapa de Regiões

```
🟢 Banco de Dados (MySQL / MariaDB)........... DESBLOQUEADA
🟢 Automação (Node.js + Baileys).............. DESBLOQUEADA
🟢 Visualização de Dados (Power BI)........... DESBLOQUEADA
🟡 Cloud & DevOps (Oracle Cloud, hardening)... EM PROGRESSO
🔴 APIs REST avançadas......................... PRÓXIMA REGIÃO
```

---

## 🏆 Badges Conquistados

![Curso](https://img.shields.io/badge/Funda%C3%A7%C3%A3o%20Bradesco-Administra%C3%A7%C3%A3o%20de%20BD-8A2BE2?style=for-the-badge&logo=bookstack&logoColor=white)
![Projeto](https://img.shields.io/badge/Bot-WhatsApp%20Financeiro-2ECC71?style=for-the-badge&logo=whatsapp&logoColor=white)
![Vaga](https://img.shields.io/badge/Candidatura-Afya-FFA500?style=for-the-badge&logo=briefcase&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-Praticando-1E90FF?style=for-the-badge&logo=mysql&logoColor=white)

---

## 🎬 Projeto em Destaque: `whatsapp-financeiro`

> Bot de controle financeiro do casal, direto pelo WhatsApp.

**Stack:** Node.js · Baileys · MariaDB · Power BI

```
Mensagem no WhatsApp
   "gasto 45 credito alimentacao mercado"
        │
        ▼
   [ parser.js ]  →  interpreta tipo, valor, categoria
        │
        ▼
   [ db.js ]      →  grava em financas_casal.lancamentos
        │
        ▼
   [ Power BI ]   →  dashboard atualizado
```

📍 Status: código e schema prontos → deploy em Oracle Cloud Always Free (Ubuntu 22.04 ARM), com hardening completo (SSH key-only, UFW, Fail2ban, backups criptografados).

---

## 🛠️ Inventário de Habilidades

![Node.js](https://img.shields.io/badge/-Node.js-339933?style=flat-square&logo=node.js&logoColor=white)
![MySQL](https://img.shields.io/badge/-MySQL%2FMariaDB-4479A1?style=flat-square&logo=mysql&logoColor=white)
![Power BI](https://img.shields.io/badge/-Power%20BI-F2C811?style=flat-square&logo=powerbi&logoColor=black)
![Git](https://img.shields.io/badge/-Git-F05032?style=flat-square&logo=git&logoColor=white)

---

## 📊 Estatísticas do Grimório (GitHub Stats)

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=SEU_USUARIO&show_icons=true&theme=tokyonight&hide_border=true" />
</p>

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=SEU_USUARIO&theme=tokyo-night&hide_border=true" />
</p>

---

<p align="center"><i>Continua... a próxima região aguarda 🗝️</i></p>
